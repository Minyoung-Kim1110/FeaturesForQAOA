...
def qaoa_maxcut(graph, n_wires: int, net: DenseNet, n_layers=1):
    ...
    def objective(net: DenseNet):
        neg_obj = 0
        dists = [[0 for _ in range(n_wires)] for _ in range(n_wires)]
        zzs = [[0 for _ in range(n_wires)] for _ in range(n_wires)]
...
bitstrings1 = qaoa_maxcut(graph, n_wires, net, n_layers=3)[1]
